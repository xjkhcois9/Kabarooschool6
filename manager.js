...
// حساب Firebase ثانوي لإنشاء حسابات الطلاب من لوحة المدير
// دون تسجيل خروج المدير الحالي.
const studentAuthApp = initializeApp(firebaseConfig, "studentAccountCreator");
const studentAuth = getAuth(studentAuthApp);
...
function renderStudents(){
  ...
  $("#studentsTable").innerHTML=rows.length?rows.map(s=>`<tr><td><code>${escapeHTML(s.studentCode||"-")}</code></td><td>${escapeHTML(s.studentId||s.id)}</td><td><strong>${escapeHTML(s.name)}</strong></td><td>${escapeHTML(s.className||s.classId||"-")}</td><td>${escapeHTML(s.sectionName||s.sectionId||"-")}</td><td>${escapeHTML(s.phone||"-")}</td><td><span class="badge active">نشط</span></td><td><div class="actions"><button class="small-btn" onclick="editStudent('${s.id}')">تعديل</button><button class="small-btn" onclick="expelStudent('${s.id}')">فصل</button></div></td></tr>`).join(""):`<tr><td colspan="8"><div class="empty">لا توجد نتائج.</div></td></tr>`;
}
...
function studentLoginEmail(code){
 const safe=String(code||"").trim().toUpperCase().replace(/[^A-Z0-9_-]/g,"");
 return `${safe}@students.school.local`;
}
function studentForm(s={}){
 ...
 const isNew=!s.id;
 return `<h2>${isNew?"إضافة طالب":"تعديل طالب"}</h2>
 <div class="form-grid">
  <label>كود دخول الطالب${isNew?`<input id="sCode" placeholder="مثال: STU1001" autocomplete="off">`:`<input value="${escapeHTML(s.studentCode||"")}" disabled>`}</label>
  <label>كلمة المرور المؤقتة${isNew?`<input id="sPassword" type="password" minlength="6" placeholder="6 أحرف/أرقام على الأقل">`:`<input value="••••••••" disabled>`}</label>
  <label>رقم الطالب<input id="sId" value="${escapeHTML(s.studentId||"")}"></label>
  <label>الاسم الكامل<input id="sName" value="${escapeHTML(s.name||"")}"></label>
  <label>الصف والشعبة<select id="sClass">${opts}</select></label>
  <label>الهاتف<input id="sPhone" value="${escapeHTML(s.phone||"")}"></label>
  <label>البريد الإلكتروني (اختياري)<input id="sEmail" value="${escapeHTML(s.email||"")}" type="email"></label>
  <button class="primary" id="saveStudent">${isNew?"إنشاء حساب الطالب":"حفظ التعديلات"}</button>
 </div>
 ${isNew?`<p class="muted-line">سيتم إنشاء حساب Firebase تلقائياً. الطالب سيدخل لاحقاً بالكود وكلمة المرور فقط.</p>`:
 `<p class="muted-line">كود الدخول مرتبط بحساب الطالب ولا يحتاج المدير إلى إنشاء قاعدة بيانات يدوياً.</p>`}`;
}
$("#addStudentBtn").onclick=()=>{
 ...
 $("#saveStudent").onclick=async()=>{
  ...
  const c=state.classes.find(x=>x.id===$("#sClass").value);
  const name=$("#sName").value.trim();
  if(!code||!/^[A-Z0-9_-]{4,30}$/.test(code))return toast("كود الطالب يجب أن يكون 4-30 حرفاً/رقماً إنجليزياً.");
  if(!password||password.length<6)return toast("كلمة المرور يجب أن تكون 6 أحرف/أرقام على الأقل.");
  if(!name)return toast("أدخل اسم الطالب.");
  if(!c)return toast("اختر الصف والشعبة.");
  try{
   const credential=await createUserWithEmailAndPassword(studentAuth,studentLoginEmail(code),password);
   const uid=credential.user.uid;
   const studentRef=doc(collection(db,"students"));
   await setDoc(studentRef,{
    authUid:uid,studentCode:code,studentId:$("#sId").value.trim(),name,
    classId:c.id,className:c.name||c.className||"",
    sectionId:c.sectionId||c.id,sectionName:c.sectionName||c.section||"",
    phone:$("#sPhone").value.trim(),email:$("#sEmail").value.trim(),
    status:"active",createdAt:serverTimestamp()
   });
   await setDoc(doc(db,"users",uid),{
    name,email:$("#sEmail").value.trim()||studentLoginEmail(code),
    role:"student",active:true,studentCode:code,studentDocId:studentRef.id,createdAt:serverTimestamp()
   });
   await authSignOut(studentAuth);
   closeModal();toast("تم إنشاء حساب الطالب بنجاح");await loadData();
  }catch(e){
   console.error(e);
   try{await authSignOut(studentAuth)}catch(_){}
   const map={
    "auth/email-already-in-use":"هذا الكود مستخدم مسبقاً. اختر كوداً آخر.",
    "auth/invalid-password":"كلمة المرور غير صالحة.",
    "auth/network-request-failed":"تعذر الاتصال بالإنترنت."
   };
   toast(map[e.code]||`تعذر إنشاء حساب الطالب: ${e.message||"خطأ غير معروف"}`);
  }
 }};
};
window.editStudent=async(id)=>{
 const s=state.students.find(x=>x.id===id);if(!s)return;
 showModal(studentForm(s));
 $("#saveStudent").onclick=async()=>{
  const c=state.classes.find(x=>x.id===$("#sClass").value);
  if(!c||!$("#sName").value.trim())return toast("أكمل بيانات الطالب.");
  await updateDoc(doc(db,"students",id),{
   studentId:$("#sId").value.trim(),name:$("#sName").value.trim(),
   classId:c.id,className:c.name||c.className||"",
   sectionId:c.sectionId||c.id,sectionName:c.sectionName||c.section||"",
   phone:$("#sPhone").value.trim(),email:$("#sEmail").value.trim(),updatedAt:serverTimestamp()
  });
  if(s.authUid){
   await setDoc(doc(db,"users",s.authUid),{
    name:$("#sName").value.trim(),email:$("#sEmail").value.trim()||studentLoginEmail(s.studentCode),updatedAt:serverTimestamp()
   },{merge:true});
  }
  closeModal();toast("تم تحديث الطالب");await loadData();
 }};
